...
async def on_join_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=user.id,
        text="📌 Bienvenido a Trader ROLO VIP BOTS by OKX. Envíame tu UID de OKX (solo números) para validar acceso."
    )
...
async def send_welcome(context, user):
    await context.bot.send_message(
        ...
        text=(
            ...
            "Aquí encontrarás bots exclusivos, estrategias avanzadas, tips de trading profesional "
            "y beneficios por pertenecer a nuestra comunidad.\n\n"
            "Además tendrás soporte personalizado en OKX.\n\n"
            "¡Vamos con todo! 🚀"
        ),
...
async def weekly_admin_report(context: ContextTypes.DEFAULT_TYPE):
    await generate_admin_report(context, "📊 REPORTE SEMANAL ADMIN - ROLO")

async def monthly_admin_report(context: ContextTypes.DEFAULT_TYPE):
    now = datetime.now(TZ_AR)
    if now.day != 30:
        return
    await generate_admin_report(context, "📊 REPORTE MENSUAL ADMIN - ROLO")
...
def main():
    ...
    print("🤖 BOT TRADER ROLO iniciado.")
    app.run_polling()
